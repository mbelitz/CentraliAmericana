...
source('/srv/duo/mbelitz/AfroTropics/R/00_setup.R')
...
occs <- data.table::fread("data/150_species_test_run_from_ODA.csv") %>% 
...
r <- list.files("out/", pattern = "_map.png")
r_spp <- word(r, 1, 2,sep = fixed("_")) %>% 
  str_replace("_", " ") %>% 
  unique()

spp_list %in% r_spp

df <- data.frame(species = spp_list, modeled = spp_list %in% r_spp)

# need to model
spp_toModel <- as.character(filter(df, modeled == FALSE)$species)
#### Step 2: Pass pipeline through the species_season list

sdm_pipeline <- function(x){
  
  print("=========================================================")
  print(paste("============== Working on", spp_toModel[x], "=============="))  
  print("=========================================================")
  binomial <- spp_toModel[x]
  species_df <-   dplyr::filter(as.data.frame(occs), bw == binomial) %>% 
    filter(!is.na(x),
           !is.na(y))
  stopifnot(nrow(species_df) >= 1)
  
  # Clip all potential predictor variables based on all cleaned occurrence
  # records from this species.
  
  aa_shp <- define_accessibleArea(species_df = species_df, minBuff = 100e3,
  ...
  max_model <- maxent(x = mymod_vars, p = coordinates(species_df), progress = "text", silent = TRUE) 
  ...
  save_SDM_results(ENMeval_output = eval1, AUCmin = 0.7, resultDir = wd$out,
                   spp = bw, occ_df = sp_df2, species_df = species_df)
...
for(i in 1:25){
  tryCatch(sdm_pipeline(x = i),
           error = function(e) print(paste(spp_toModel[i], "Error in Code Skipping for now!")))
}  

sdm_pipeline(2)

## generate maps for species where .png didn't save
generate_maps <- function(x){
  
  binomial <- x
  
  print(binomial)
  
  bw <- str_replace(x, pattern = " ", "_")
  resultDir <-  wd$out
  ### 4C. Visualize the model
  ## Load in the rasters
  r    <- raster(file.path(resultDir, paste0(bw,"_SDM.tif")))
...
generate_maps(spp_list[1])

generate_maps("Aciagrion steeleae")


# looping through pipeline
for(i in 2:155){
  tryCatch(generate_maps(x = spp_list[i]),
           error = function(e) print(paste(spp_list[i], "Error in Code Skipping for now!")))
}  


a <- list.files("thresholdingOut/")
